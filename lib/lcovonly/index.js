...
class LcovOnlyReport extends ReportBase {
  ...
  onDetail(node) {
    ...
    Object.entries(functionMap).forEach(([key, meta]) => {
      ...
      // clear line coverage for uncalled single line methods
      if (stats == 0 && meta.decl.start.line == meta.loc.end.line)
        lines[meta.decl.start.line] = 0;
    ...
    Object.entries(branches).forEach(([key, branchArray]) => {
      ...
      // clear line coverage for missing branch coverage
      if (branchArray.some(calls => !calls)) {
        lines[meta.loc.start.line] = 0;
      }
    });
